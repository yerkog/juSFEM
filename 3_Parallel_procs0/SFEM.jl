module SFEM
    ...
    include("G://Project-SFEM//3_Parallel_procs0//assembleout.jl")
    include("G://Project-SFEM//3_Parallel_procs0//assemblein.jl")
...
======================================================================#
    function Ksfem(Node, Ele, Cen, Face_out, Face_in, EE, NU, FF)
        ...
        IK = zeros(Int64,Int(size(Face_in, 1)*225)+Int(size(Face_out, 1)*144))
        JK = zeros(Int64,Int(size(Face_in, 1)*225)+Int(size(Face_out, 1)*144))
        VK = zeros(Float64,Int(size(Face_in, 1)*225)+Int(size(Face_out, 1)*144))
